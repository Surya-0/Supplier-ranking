...
def main():
    ...
    if "graph" not in st.session_state:
        ...
        st.session_state.supply_chain = SupplyChainGraph()
    ...
    with st.sidebar:
        ...
        if st.button("Fetch Data"):
            with st.spinner("Fetching data..."):
                data = st.session_state.supply_chain.fetch_data(
                    base_url, version, timestamp
                )
                if data:
                    st.session_state.graph = st.session_state.supply_chain.build_graph()
        ...
        if use_subgraph and st.session_state.graph:
            # Get available node types
            node_types = list(
                set(nx.get_node_attributes(st.session_state.graph, "type").values())
            )
            ...
            if source_node_types or source_node_ids:
                ...
                temp_supply_chain.G = st.session_state.graph
    ...
    if st.session_state.graph is not None:
        ...
        if use_subgraph:
            current_graph = st.session_state.supply_chain.create_query_subgraph(
                source_node_types=source_node_types,
                source_node_ids=source_node_ids,
                n_hops=n_hops,
